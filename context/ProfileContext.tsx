...
export const ProfileProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const logout = useCallback(async () => {
    ...
    try {
      ...
      if (supabase) {
        try {
          // Race the sign out against a 1.5s timeout
          await Promise.race([
            supabase.auth.signOut(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Logout timeout')), 1500))
          ]);
        } catch (authError) {
          console.error('Sign out call issue (proceeding with local clear):', authError);
        }
      }

      // ALWAYS clear local state and redirect, even if server-side sign out failed
      setProfile(null);
      setSession(null);
      setError(null);

      // Crucial: Clear all local storage/session storage to be safe
      try {
        localStorage.clear();
